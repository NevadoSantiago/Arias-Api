...
| 11 | `CreditPack` + `CreditPurchase` + webhook + reconciliación | 11 | `MercadoPagoWebhookControllerTest` (duplicado, reversión) | Mock del SDK + fixtures de payload firmados | Revertir migración V22 + paquete `payments` (purchase/webhook) |
...
- [x] 11.1 Migración `backend/src/main/resources/db/migration/V22__credit_packs_and_purchases.sql` (tablas `credit_pack`, `credit_purchase`, `UNIQUE(mp_payment_id)`). Además de la tabla del diseño, se agregó `credit_purchase.credits_reversed` (ver nota de 11.5). La migración se numeró **V22** en vez de la V19 reservada por el diseño: V20 y V21 ya estaban aplicadas, y renumerar evita activar `flyway.out-of-order`, que desactivaría la validación de orden de forma permanente, también en producción.
- [x] 11.2 Crear `backend/src/main/java/com/arias/credits/packs/{CreditPack,CreditPackRepository,CreditPackService,AdminCreditPackController,CreditPackDto,CreateCreditPackRequest,UpdateCreditPackRequest,CreditPackController}.java` (CRUD `SUPER_ADMIN` en `/api/v1/admin/credit-packs`, `price_cents` autoritativo, `discount_percent` solo presentación; `CreditPackController` agregado para el catálogo público `GET /api/v1/credits/packs` que el diseño lista en "Endpoints expuestos" pero no asignaba a ningún archivo).
- [x] 11.3 Crear `backend/src/main/java/com/arias/payments/{CreditPurchase,PurchaseType,CreditPurchaseStatus,CreditPurchaseRepository,CreditPurchaseService,CreditPurchaseController,CreatePurchaseRequest,CreditPurchaseDto,CreditPurchaseCheckoutDto}.java`: `POST /api/v1/credits/purchases`, importe calculado en servidor, `external_reference = credit_purchase.id`. **Nota de deviación (compra directa)**: el diseño no define una tarifa de créditos independiente del catálogo de paquetes; el precio unitario de una compra directa se deriva de `price_cents / credit_amount` del paquete `code = "DAY"` habilitado (la denominación más chica) — si no existe, el endpoint responde 503 `direct-purchase-unavailable` en vez de adivinar un precio. Requiere confirmación de producto. También sin resolver: el diseño dice que un pago rechazado/cancelado de una compra directa "cancela el pedido asociado" — no implementado (el pedido queda simplemente sin pagar, `PENDIENTE`), documentado en `CreditPurchaseService#closeWithoutCrediting`.
- [x] 11.4 Crear `backend/src/main/java/com/arias/payments/MercadoPagoWebhookController.java`: los 9 pasos del diseño (validar firma → `topic != payment` → `PaymentClient.get` → resolver `credit_purchase` → comparar importe → `SELECT FOR UPDATE` con atajo de "ya acreditada" → mapear estado → commit → email `AFTER_COMMIT` vía `PaymentEmails`/`CreditPurchaseCreditedEvent`/`CreditPurchaseReversedEvent`, mismo patrón que la unidad 12).
- [x] 11.5 Aplicar mapeo de estados del diseño: `approved` → `PACK_PURCHASE` (renueva `expires_at`) o `DIRECT_PURCHASE` (sin renovar, directo a COMMITTED); `rejected/cancelled` → cierra sin acreditar; `refunded/charged_back` → `PAYMENT_REVERSAL` acotado a `available` (nunca la deja negativa — `CreditLedgerService.reverse`, nuevo método additivo), sin tocar `expires_at`.
  **Resolución de la unidad 9 (decisión del orquestador, reemplaza el checklist manual de sandbox que no pudo ejecutarse — credenciales de test vs. pagos solo en modo live)**: la reversión se calcula **proporcional al monto reembolsado**, nunca a partir del string de estado: `reverse = round(creditsGranted * transactionAmountRefunded / transactionAmount)`, acumulando en `credit_purchase.credits_reversed` cuánto de esa compra ya se revirtió, y aplicando en cada notificación solo el DELTA contra ese acumulado (`targetTotal - creditsReversed`) — así un reembolso parcial repetido/encadenado nunca revierte de más, y el acumulado nunca supera `credit_amount`. Esto es correcto tanto si un reembolso parcial deja el pago `approved` con `transaction_amount_refunded` poblado (por eso `CreditPurchaseService#applyStatusMapping` revisa el reembolso también en la rama `APPROVED`, no solo en `REFUNDED/CHARGED_BACK`) como si pasa a `refunded`. Un `charged_back` sin `transaction_amount_refunded` poblado se trata como reversión total (los contracargos no siempre reportan ese campo). Implementado en `PaymentSnapshot.amountRefundedCents` (campo nuevo, poblado desde `Payment.getTransactionAmountRefunded()` del SDK) y `CreditPurchaseService#maybeReverse`. Cubierto por `MercadoPagoWebhookControllerTest#reembolsoParcialRevierteProporcionalmente` (reembolso 50% → revierte 5 de 10; mismo webhook repetido → delta 0, no revierte de nuevo; reembolso al 100% → revierte el delta restante y recién ahí marca `REVERSED`).
- [x] 11.6 Crear `backend/src/main/java/com/arias/payments/PaymentReconciliationScheduler.java`: `PENDING` > 30 min re-consulta vía `PaymentGateway.findByExternalReference` (método nuevo del puerto, unidad 10 extendida additivamente con `PaymentClient.search`); > 24 h → `EXPIRED`.
- [x] 11.7 Test: `MercadoPagoWebhookControllerTest` (6 tests) — webhook duplicado no acredita dos veces (contra la restricción real `UNIQUE(mp_payment_id)`), firma inválida rechaza sin acreditar, redirección del navegador sola no acredita, compra directa = un único movimiento, importe que no coincide rechaza, reembolso parcial revierte proporcionalmente — cubre spec `credit-pack-purchase` completa. Más `CreditPurchaseServiceTest` (4 tests) — cálculo de importe en servidor para paquete y compra directa, rechazo sin paquete DAY, rechazo de pedido ajeno. `PaymentGateway` mockeado con `@MockitoBean`, ninguna llamada real a Mercado Pago. `./mvnw test`: 141/141 verdes (131 previos + 10 nuevos), incluye `OrderServiceCompanyFlowTest` (8) y `BillingServiceTest` (16) sin ediciones.
